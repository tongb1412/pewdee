...
$cl = $color1;
$sql = "select a.*,b.cradno,b.pname,b.fname,b.lname,(a.total - a.recive) aa  from tb_payment a,tb_patient  b,tb_vst c where (a.hn = b.hn) and (a.vn=c.vn) and (a.pdate between '$sdate%' and '$edate%')  and (c.status='COM') " . $where_branch_id;
...
?>
	...
	<div class="list_out" onmouseover="linkover(this)" onmouseout="linkout(this,'<?= $cl ?>')" style="width:98%;;background:<?= $cl ?>; ">
		...
		<div style="width:15%; float:left;"><?= number_format($rs['total'] -  $rs['discount'], '0', '.', ',') ?></div>
...
include('../class/config.php');
$dat = date('d-m-Y',time());
$sql = "select sum(a.cash) s_cash ,sum(a.credit) s_credit,sum(a.recive) s_total from tb_payment a,tb_vst b where (a.pdate between '$sdate%' and '$edate%') and  (a.vn=b.vn) and (b.status='COM') " . $where_branch_id; 
// echo $sql;exit();
...
?>
	...
	<div class="line" style="margin-top: 4px;">
		...
		<div style="width:10%; float:left;">
			<input style="font-weight:bold; text-align:right;" name="text2" type="text" id="" size="12" ; value="<?= number_format($row['s_total'], '0', '.', ',') ?>" />
		</div>
	</div>
