...
if(empty($did)){
	if($sdate == $edate){
		$sql = "select a.did,a.dname, a.branchid,sum(a.qty) qty,count(*) count from tb_drugerec a,tb_vst b,tb_patient c where a.vn = b.vn and b.status <> 'CANCLE'  and a.hn = c.hn  and ( b.vdate like '%$sdate%' ) $where_branch_id group by a.did,a.dname  ";	
	} else {	
		$sql = "select a.did,a.dname, a.branchid,sum(a.qty) qty,count(*) count from tb_drugerec a,tb_vst b,tb_patient c where a.vn = b.vn and b.status <> 'CANCLE'  and a.hn = c.hn  and (b.vdate between '$sdate%' and '$edate%') $where_branch_id group by a.did,a.dname  ";
	}
} else {
	if($sdate == $edate){
		$sql = "select a.did,a.dname, a.branchid,sum(a.qty) qty,count(*) count from tb_drugerec a,tb_vst b,tb_patient c where a.vn = b.vn and b.status <> 'CANCLE'  and a.hn = c.hn  and (b.vdate like '%$sdate%') and (a.did like '%$did%') $where_branch_id group by a.did,a.dname";
	} else {	
		$sql = "select a.did,a.dname, a.branchid,sum(a.qty) qty,count(*) count from tb_drugerec a,tb_vst b,tb_patient c where a.vn = b.vn and b.status <> 'CANCLE'  and a.hn = c.hn and (b.vdate between '$sdate%' and '$edate%') and (a.did like '%$did%') $where_branch_id group by a.did,a.dname  ";
...
?>
	...
	{
	?>
		<a href="javascript: ajaxLoad('post','Monthly_report/retotaldrugerec_list.php','branchid=<?php echo $branch_id ?>&Page=<?= $Prev_Page ?>&sdate=<?= $sdate ?>&edate=<?= $edate ?>&did=<?= $_POST['did'] ?>','d_list')">
	...
	?>
		...
		<a href="javascript: ajaxLoad('post','Monthly_report/retotaldrugerec_list.php','branchid=<?php echo $branch_id ?>&Page=<?= $Next_Page ?>&sdate=<?= $sdate ?>&edate=<?= $edate ?>&did=<?= $_POST['did'] ?>','d_list')">
