...
if(! empty($txt)){
$sql = "select * from tb_staff where staffid like '%$txt%' or fname like '%$txt%'   order by fname asc   limit 5";
...
?>
<div  style="width:99%; height:25px; line-height:25px; text-align:left; margin-left:1px; border-bottom:#CCCCCC 1px dotted;background:<?=$cl?>; cursor:pointer;" onmouseover="linkover(this)" onmouseout="linkout(this,'<?=$cl?>')" onclick="movesemp('<?=$rs['staffid']?>','<?=$rs['pname'].$rs['fname'].'   '.$rs['lname']?>')"  >
