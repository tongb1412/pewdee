...
var DD_roundies = {
  ...
  addRule: function (selector, radius, standards) {
    if (this.styleSheet.addRule) {
      ...
      this.styleSheet.addRule(
        selector,
        "behavior:expression(DD_roundies.roundifyElement.call(this, " +
          radius +
          "))"
      );
    } else {
  ...
  roundifyElement: function (rad) {
    ...
    this.hideSourceBorder = function () {
      ...
      for (var i = 0; i < 4; i++) {
        this.runtimeStyle["padding" + sides[i]] =
          parseInt(this.currentStyle["padding" + sides[i]]) +
          parseInt(this.realBorderWidth) +
          "px";
      }
    };
      ...
      if (
        ...
        (this.currentStyle.backgroundImage != "none" ||
          (this.currentStyle.backgroundColor &&
            this.currentStyle.backgroundColor != "transparent"))
      ) {
    ...
    this.updateVmlDimensions = function () {
      ...
      this.vml.coordorigin =
        -this.halfRealBorderWidth + " " + -this.halfRealBorderWidth;
      this.vml.coordsize =
        this.dim.w +
        this.realBorderWidth +
        " " +
        (this.dim.h + this.realBorderWidth);
    };
    ...
    this.handlePseudoHover = function () {
      setTimeout(function () {
        /* wouldn't work as intended without setTimeout */ self.runtimeStyle.backgroundColor =
          "";
        self.updateVmlFill.call(self);
        self.updateVmlStrokeColor.call(self);
      }, 1);
  ...
  if (contentType.match("text/javascript")) {
    ...
    if (mode[5] == "RG") {
      ...
      window.open(
        page,
        "INVOICE/RECEIPT",
        "width=450, height=550,resizable=yes, scrollbars=yes"
      );
    } else {
  ...
  if (contentType.match("text/javascript")) {
    ...
    if (txt[1] == "Yes") {
      ...
      cleartabreg(
        ...
        6,
        8,
        "register/patient_edit_from.php",
        "content",
        "hn=" + txt[3]
      );
    }
    ...
    if (mode[2] == "PCT") {
      ...
      if (mode[5] == "Y") {
        ...
        window.open(
          page,
          "Instock",
          "width=1003, height=600,resizable=yes, scrollbars=yes"
        );
      }
...
function oldcourse(hn, vn) {
  ...
  window.open(
    page,
    "SLIP",
    "width=800, height=370,resizable=yes, scrollbars=yes"
  );
}
...
function cleartabreg(id, n, m, URL, displayId, data) {
  // ฟังก์ชั่นนี้ คือการปิด และ เปิด Tab ตั้งแต่ Tab ที่ 1 จนถึง Tab ที่เกี่ยวข้องกับตัวฟังก์ชั่น
  var i = 1;
  pHN = data;
  while (i < m + 1) {
...
function chk_dob(line) {
  if (line == 1) {
    if (document.getElementById("dd").value.length > 1) {
      document.getElementById("dm").focus();
    }
  } else {
    if (document.getElementById("dm").value.length > 1) {
...
function totalcash() {
  var sum =
    parseFloat(document.getElementById("cash_yes").value.replace(",", "")) +
    parseFloat(document.getElementById("to_day").value.replace(",", ""));

  var dis =
    parseFloat(document.getElementById("coste").value.replace(",", "")) +
    parseFloat(document.getElementById("doctor").value.replace(",", "")) +
    parseFloat(document.getElementById("bank").value.replace(",", ""));
...
function changemoney(n) {
  ...
  if (mm > 0) {
    document.getElementById("cash").value = formatMoney(mm);
  } else {
    document.getElementById("cash").value = "0.00";
  }
  if (mc > 0) {
    document.getElementById("credit").value = formatMoney(mc);
  } else {
    document.getElementById("credit").value = "0.00";
  }
  if (mk > 0) {
    document.getElementById("kupong").value = formatMoney(mk);
  } else {
    document.getElementById("kupong").value = "0.00";
  }
...
function printpaytotal() {
  ...
  window.open(
    page,
    "Patients",
    "width=1003, height=500,resizable=yes, scrollbars=yes"
  );
}
