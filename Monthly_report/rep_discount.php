...
if(empty($did)){
$sql = "select a.*,b.cradno,b.pname,b.fname,b.lname,c.empid,c.empname, d.cn, d.clinicname from tb_payment a,tb_patient b,tb_vst c, tb_clinicinformation d  where (a.hn = b.hn) and (a.vn=c.vn)  and (a.pdate  between '$sdate%' and '$edate%') and (c.status='COM') and a.discount <> 0 and a.total = a.discount and a.branchid = d.cn ";
} else {
$sql = "select a.*,b.cradno,b.pname,b.fname,b.lname,c.empid,c.empname, d.cn, d.clinicname from tb_payment a,tb_patient b,tb_vst c, tb_clinicinformation d  where (a.hn = b.hn) and (a.vn=c.vn)  and (a.pdate  between '$sdate%' and '$edate%') and (c.empid like '%$did%') and (c.status='COM') and a.discount <> 0 and a.total = a.discount and a.branchid = d.cn ";
}
...
if($s=='y'){ 	
	...
	<div style="width:100%; height:30px; line-height:25px; text-align:center; font-size:10px; font-weight:bold;  float:left; ">
		...
		<div style="width:23%; float:left; border-bottom:#999999 2px solid;">ชื่อ-สกุล</div>
...
if($empid != $rs['empid'] ){
	...
	<div style="width:100%; font-size:10px; text-align:left; float:left; margin:auto; font-weight:bold; border-top:#CCCCCC 1px dotted; overflow:hidden;">
		...
		<div style="width:23%; float:left; text-align:center"><?= $h - 1 . '  รายการ'; ?>&nbsp;</div>
...
<div style="width:100%; font-size:10px; text-align:left; float:left; margin:auto; overflow:hidden; ">
	...
	<div style="width:23%; float:left;"><?= $rs['pname'] . $rs['fname'] . '    ' . $rs['lname'] . '  '  ?></div>
...
<div style="width:100%; font-size:10px; text-align:left; float:left; font-weight:bold; margin:auto; margin-top:5px;  overflow:hidden;">
	...
	<div style="width:23%; float:left; text-align:center"><?= $nn . '  รายการ'; ?>&nbsp;</div>
