...
if(empty($did)){
	if($sdate == $edate){
		$sql = "select a.*,b.cradno,b.pname,b.fname,b.lname from tb_payment a,tb_patient b,tb_vst c  where (a.hn = b.hn) and (a.vn=c.vn)  and ( a.pdate like '%$sdate%' ) and (c.status='COM')  ";	
	} else {	
		$sql = "select a.*,b.cradno,b.pname,b.fname,b.lname from tb_payment a,tb_patient b,tb_vst c  where (a.hn = b.hn) and (a.vn=c.vn)  and (a.pdate between '$sdate%' and '$edate%') and (c.status='COM')   ";
	}
} else {
	if($sdate == $edate){
		$sql = "select a.*,b.cradno,b.pname,b.fname,b.lname  from tb_payment a,tb_patient b,tb_vst c  where (a.hn = b.hn) and (a.vn=c.vn)  and (a.pdate like '%$sdate%') and (c.empid like '%$did%') and (c.status='COM')  ";
	} else {	
		$sql = "select a.*,b.cradno,b.pname,b.fname,b.lname  from tb_payment a,tb_patient b,tb_vst c  where (a.hn = b.hn) and (a.vn=c.vn)  and (a.pdate between '$sdate%' and '$edate%') and (c.empid like '%$did%') and (c.status='COM') ";
