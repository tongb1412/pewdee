...
if($mod=='Y'){
	...
	$n = mysql_num_rows($str);
	if(empty($n)){
		$sql = "insert into tb_appointment  values('$an','$hn','$cn','$pname','$pid','$atyp','$dat','$stim','$etim','','$mem','NONE')";
		mysql_query($sql) or die ("Error Query [".$sql."]");
	} else {
		$sql = "Update tb_appointment Set atyp='$atyp',dat='$dat',stim='$stim',etim='$etim',pid='$pid',mem='$mem' Where an='$an' ";
		mysql_query($sql) or die ("Error Query [".$sql."]");
	}
