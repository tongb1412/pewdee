...
while($rs = mysql_fetch_array($result)){
    ...
    ?>
                    ...
                    <div style="width:98%; height:20px; line-height:20px; text-align:left; margin-left:5px; border-bottom:#CCCCCC 1px dotted; " onmouseover="linkover(this)" onmouseout="linkout(this,'<?= $cl ?>')">
                        ...
                        <div style="width:15%;float:left; text-align:right; "><input type="button" value="  " class="btn_del" onclick="pctUseDelete('doctor/pctuse_del.php','content','<?= $rs['uvn'] ?>','<?= $rs['tid'] ?>','<?= $rs['empid'] ?>','<?= $rs['ftyp'] ?>',<?= $rs['qty'] ?>,'<?= $rs['pid'] ?>','<?= $rs['vn'] ?>')" title="" alt="" /> </div>
...
?>
    ...
    <div style="width:53%; height:470px; float:left;">
        <div style="width:100%; height:30px; line-height:30px; font-weight:bold; font-size:18px; border-bottom:#CCCCCC 1px dotted; text-align:left;">
            ...
            <div style="width:20%; float:left;"><?= ' : ' . $age . ' ' ?></div>
        </div>
        <div style="width:100%; height:400px; background:#fffaf0; ">
            ...
            <div class="line" style="height:auto; border:1px <?= $tabcolor ?> solid;">
                <div style="width:100%; height:20px; color:#000000; margin:auto; font-weight:bold; font-size:13px; background:<?= $tabcolor ?>;">
                    <div style="width:75%;text-align:left; float:left; line-height:20px;">
                        <img src="images/icon/bullet_arrow_down.png" align="absmiddle" />&nbsp;
                    </div>
		    ...
		    $sql_pct = "select a.* from tb_pctrec a,tb_vst b  where a.hn='$hn' and total > 0 and a.vn=b.vn and a.hn = b.hn and b.status <>'CANCEL'  ";
				...
				$unit = $rs_pct['unit'];
		   		...
		   		if($type=='L' || $type=='T'){
					...
					if( $tqty > $uqty){
						...
						if(empty($n)){
							$tqty = $qty  -  $uqty;
							$sql = "insert into tb_pctlist  values('$pvn','$hn','$pid','$pname','$tqty','$unit','$type')";
							mysql_query($sql);
						} else {
							$rr = mysql_fetch_array($tr);
							$tqty = $qty +  $rr['qty'] - $uqty;
							$sql = "update tb_pctlist set qty='$tqty' where vn='$pvn' and hn='$hn' and tid='$pid'";
							mysql_query($sql);
						}
					}
        ...
        <div style="width:100%; height:40px; text-align:right; background:#f5f5f5;">
            ...
            <div style="width:100%; float:left; text-align:right;">
                <input type="button" value="    " style="font-size:14px; font-weight:bold; height:40px;" onclick="addvst('doctor/add_vst.php','FIN')" />
                <input type="button" value="    " style="font-size:14px; display:none; font-weight:bold; height:40px;" onclick="oldcourse('<?= $hn ?>','<?= $vn ?>')" />
                <input type="button" value="       " style="font-size:14px; font-weight:bold; height:40px;" onClick="swabtab(1,2,'doctor/doctor.php','home','')" />
            </div>
        </div>
