...
?>
			<div style="width:98%; height:20px; padding-top:5px; color:#000000; margin:auto; font-weight:bold; font-size:12px; background:<?=$tabcolor?>;">
				...
				<div style="width:45%;text-align:left; float:left;">&nbsp;<img src="images/icon/bullet_arrow_down.png" align="absmiddle" />&nbsp;ชื่อ-สกุล</div>
...
$sql .=" order by hn asc LIMIT $Page_Start , $Per_Page";
...
<div class="list_out" onmouseover="linkover(this)" onmouseout="linkout(this,'<?=$cl?>')" style="width:99%;background:<?=$cl?>; ">
	...
	<div style="width:25%; height:20px; text-align:left; line-height:20px;  float:left; "><?=$rs['datep']?></div>
	<div style="width:15%; height:20px; text-align:left; line-height:20px;  float:left;">&nbsp;<?=$rs['cradno']?></div>
	<div style="width:45%; height:20px; text-align:left; line-height:20px;  float:left;">&nbsp;<?=$rs['pname'].$rs['fname'].'    '.$rs['lname']  ?></div>
...
<div style="width:83%; margin:auto; margin-top:10px; text-align:right; line-height:20px;">
 <?=$Num_Rows;?> 
  ...
  <?
	...
	{
	?>
	<a href="javascript: ajaxLoad('post','Monthly_report/renewpatient_list.php','Page=<?=$Prev_Page?>&sdate=<?=$sdate?>&edate=<?=$_POST['edate']?>','pa_list')">	
	...
	<a href="javascript: ajaxLoad('post','Monthly_report/renewpatient_list.php','Page=<?=$Next_Page?>&sdate=<?=$sdate?>&edate=<?=$_POST['edate']?>','pa_list')">	
...
?>	
	...
	<div class="line" style="margin-top: 4px;">
      ...
      <div style="width:15%; float:left; text-align:right;">จำนวนคนไข้ใหม่ :&nbsp;</div>
