...
$cl = $color1;
$sql = "select a.*,b.cradno,b.pname,b.fname,b.lname,(a.total - a.recive) aa  from tb_payment a,tb_patient  b where (a.hn = b.hn) and  (a.pdate like '%$dat%') ";
...
<div class="list_out" onmouseover="linkover(this)" onmouseout="linkout(this,'<?=$cl?>')" style="width:98%;;background:<?=$cl?>; ">
	...
	<div style="width:15%; float:left;"><?=number_format($rs['total'] -  $rs['discount'],'2','.',',')?></div>
...
include('../class/config.php');
$dat = date('d-m-Y',time());
$sql = "select pdate,sum(cash) s_cash ,sum(credit) s_credit,sum(recive) s_total from tb_payment where (pdate like '%$dat%') group by 'pdate%' ";
