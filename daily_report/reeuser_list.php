...
?>
	...
	{
	?>
		<a href="javascript: ajaxLoad('get','daily_report/reeuser_list.php','branchid=<?php echo $branch_id; ?>&did=<?php echo $did; ?>&mode=<?= $mode ?>&Page=<?= $Prev_Page ?>','d_list')">
	...
	{
	?>
		<a href="javascript: ajaxLoad('get','daily_report/reeuser_list.php','branchid=<?php echo $branch_id; ?>&did=<?php echo $did; ?>&mode=<?= $mode ?>&Page=<?= $Next_Page ?>','d_list')">
...
if(empty($did)){
	...
	$sql  = "select sum(((a.totalprice / a.qty)*b.qty)) s_total";
	$sql .= "from tb_pctrec a,tb_pctuse b,tb_patient c  where (a.hn = c.hn) and  (b.dat like '%$dat%') and (a.vn = b.vn) and (b.empid like '%$did%') and (a.tid = b.pid)";
}
