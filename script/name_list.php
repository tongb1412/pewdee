...
include('../class/config.php');
    ...
    if($fn == "get_doctor_data"){
        ...
        $data = array();
        if($branch_id == "00"){
            $sql = "select staffid,pname,fname from tb_staff ORDER BY fname";

        } else {
            $sql = "select staffid,pname,fname from tb_staff where branchid = '$branch_id' ORDER BY fname";
        }
        $result = mysql_query($sql) or die ("Error Query [".$sql."]");
        $Num_Rows = mysql_num_rows($result);
        if(!empty($Num_Rows)){
